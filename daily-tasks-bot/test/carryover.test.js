...
test("a pending task carries over day after day, staying one row", async () => {
  ...
  const created = await taskService.addTask(store, "2026-08-10", {
  ...
  let grouped = await taskService.getTodayGrouped(store, "2026-08-11");
  ...
  grouped = await taskService.getTodayGrouped(store, "2026-08-12");
  assert.equal(grouped.carriedOver.length, 1);
  ...
  // Still exactly one row in the whole store -- no duplication.
...
test("completing a task stops it from carrying over", async () => {
  ...
  await taskService.completeTask(store, created.id);

  const grouped = await taskService.getTodayGrouped(store, "2026-08-13");
  assert.equal(grouped.carriedOver.length, 0);
  assert.equal(grouped.newToday.length, 0);

  const stillThere = await taskService.findTaskById(store, created.id);
...
test("a task due today is 'new today', not carried over", async () => {
  const store = createMemoryStore();
  await taskService.addTask(store, "2026-08-12", {
    title: "Review sales reps",
    dueDate: "2026-08-12",
    chatId: 111,
  });

  const grouped = await taskService.getTodayGrouped(store, "2026-08-12");
...
test("manual 'tomorrow' reschedule moves due_date without touching carryover_count", async () => {
  const store = createMemoryStore();
  const created = await taskService.addTask(store, "2026-08-12", {
    title: "Approve Instagram content",
    dueDate: "2026-08-12",
    chatId: 111,
  });

  const rescheduled = await taskService.rescheduleTomorrow(store, "2026-08-12", created.id);
...
test("a deleted task never reappears, even across day rollovers", async () => {
  ...
  await taskService.deleteTask(store, created.id);

  const grouped = await taskService.getTodayGrouped(store, "2026-08-14");
  assert.equal(grouped.carriedOver.length, 0);
  assert.equal(grouped.newToday.length, 0);
});
...
test("digest idempotency flag: marking sent then checking again for the same day", async () => {
  const store = createMemoryStore();
  assert.equal(await taskService.wasDigestSentToday(store, "2026-08-12"), false);
  await taskService.markDigestSentToday(store, "2026-08-12");
  assert.equal(await taskService.wasDigestSentToday(store, "2026-08-12"), true);
  assert.equal(await taskService.wasDigestSentToday(store, "2026-08-13"), false);
});
